...
def predict_backscatter(var, lai, m, sm):
    ...
    tau_sq = np.exp(b * lai)
    # tau_sq = 1 + b * lai
    e_s = -76.6 * (sm ** 3) + 146 * (sm ** 2) + 9.3 * sm + 3.03
    # e_s = (Ax + Bx * sm) ** 2
    theta = 2 * np.pi / 9
    ...
    dub_vh = m * 10 ** (-2.68) * ((np.cos(theta) ** 1.45) / (np.sin(theta) ** 2.4)) * 10 ** (0.013 * e_s * (1 / np.tan(theta))) * (
            k * s * np.sin(theta)) ** 1.1 * (c / f) ** (-0.53)
    ...
    s_veg = (1 - m) * a * (lai ** (e + 1))
    s_inter = lai * (2 * np.sqrt(tau_sq) + tau_sq) * s_veg * dub_vh

    model = s_veg + dub_vh + s_inter
    model = 10 * np.log10(model)
    return model


def residual(var, lai, m, sm, svh):
    y = predict_backscatter(var, lai, m, sm)
    return svh - y


def residual_lai(lai, var, m, sm, svh):
    y = predict_backscatter(var, lai, m, sm)
    return svh - y


def residual_sm(sm, var, lai, m, svh):
    y = predict_backscatter(var, lai, m, sm)
    return svh - y
...
out = least_squares(residual, x0=variables,
                    args=(lai_data1, dop_data1, sm_data1, vh_data1),
                    method='lm')
test = residual(out.x, lai_data1, dop_data1, sm_data1, vh_data1)
model_f = predict_backscatter(out.x, lai_data1, dop_data1, sm_data1)
# vh_data1 = refine(model_f.reshape(-1, 1), vh_data1.reshape(-1, 1), 0.9).flatten()
print("Optimised Parameters are: {}".format(out.x))
print("model is {}".format(model_f))
print("svh is {}".format(vh_data1))
...
up = np.stack((vh_data1, model_f), axis=1)
df = pd.DataFrame(up, columns=['Svh', 'Model'])
df.to_excel('Up.xlsx')


pearson_r, pearson_p = pearsonr(vh_data1, model_f)
...
for i in range(len(sm_guess)):
    if sm_guess[i] == 0:
        sm_guess[i] = 0.5


inv1 = least_squares(residual_sm, x0=sm_guess, args=(out.x, lai_data1[:10], dop_data1[:10], vh_data1[:10]),
                     method='trf')
sm_p = inv1.x
...
inv1 = least_squares(residual_lai, x0=lai_guess, args=(out.x, dop_data1[:10], sm_data1[:10], vh_data1[:10]),
                     method='trf')
lai_p = inv1.x
...
plt.scatter(vh_data1, model_f, c=(vh_data1 - model_f), cmap='Spectral', edgecolor='k', s=200, alpha=0.7, linewidths=1.5,
            marker='.')
plt.plot(dpi=600)
plt.plot(vh_data1, model_f, 'o')
slope, d = np.polyfit(vh_data1, model_f, 1)
plt.plot(vh_data1, slope * vh_data1 + d)
